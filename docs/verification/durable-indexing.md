...
This single-entry smoke check does not establish relevance, the 1,000-entry workload, the 60-second freshness target under load, or the 30-minute rebuild gate. Those measurements remain outstanding. Model identity is currently the configured model name; digest/input-version compatibility and model-change coordination require further implementation. Metadata maintenance must still schedule revision work in its own publication transaction.

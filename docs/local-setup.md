...
For production browser measurements, build with `npm run build --workspaces --prefix frontend`, then run `npm run test:performance --workspace=@quarry/app --prefix frontend`. These separate benchmarks perform 20 fresh loads and 120 loaded interaction changes under the required network/CPU profile using a 1,000-entry HTTP mock and the real illustrative preview; no SQL or Ollama service is needed. See [browser performance evidence](verification/browser-performance.md) for reports, the manual workflow, and remaining performance gates.
